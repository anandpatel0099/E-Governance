/*for displaying calendar*/
		function openCalendar(val)
		{
		    //parameter val contains the name,id of the textbox
			displayCalendar(document.getElementById(val),"dd/mm/yyyy",document.getElementById(val));
		}
		

var languageCode = 'en';	// en = english, gu = hindi


// document.getElementById(val).focus();rmat of current day at the bottom of the calendar
...
var pathToImages = 'images/cal/aev_';	// Relative to your HTML file
...
var selectBoxHighlightColor = '#D60808'; // Highlight color of select boxes - D60808

var selectBoxRolloverBgColor = '#E2E2E2'; // Background color on drop down lists(rollover) - E2EBED
...

{
    ...
    // Display only 2 years

    for(var no=startYear;no<(startYear+2);no++){
...

{
	...
	if(inputField.value.length==format.length || inputField.value.length==(format.length - 2)){
        ...
        // If 2 digit year entered, convert to 4 digit.

        if(currentYear.length == 2){

          currentYear = setYear4Digit(currentYear);
...
function setYear4Digit(strYear){
